...
class VacuumPlanning(Problem):
    ...
    def computeTurnCost(self, action1, action):
        ...
        if action1 is None or action1 == action:
            return 0

        # storing index values
        idx1 = direction.index(action1)
        idx = direction.index(action)

        # Calculating turn cost
        TurnCost = abs(idx1 - idx)

        if TurnCost > 2:
            TurnCost = 4 - TurnCost

        # Returning the final value of turn cost
        return TurnCost * 3
    ...
    def findMinManhattanDist(self, pos):
        ...
        # ref: https://stackoverflow.com/questions/3477283/what-is-the-maximum-float-in-python (Gained knowledge)
        min_dist = sys.float_info.max
        for single_thing in self.env.things:
            if isinstance(single_thing, Dirt):
                # using distance_manhattan() function in utils.py
                dist = distance_manhattan(single_thing.location, pos)
                # comparing distances
                if dist < min_dist:
                    min_dist = dist
    ...
    def findMinEuclidDist(self, pos):
        ...
        for single_thing in self.env.things:
            if isinstance(single_thing, Dirt):
                # using distance_manhattan() function in utils.py
                dist = distance_euclid(single_thing.location, pos) ** 2
                # comparing deistences
                if dist < min_dist:
                    min_dist = dist
        if min_dist != sys.float_info.max:
            return min_dist
...
def breadth_first_graph_search(problem):
    ...
    """
    # Taking Inspiration from the lecture slides
    initial_node = Node(problem.initial)

    # If condition is true then return the initial node
    ...
    while not is_empty(frontier):
        ...
        explored.add(tuple(initial_node.state))

        # going through all nodes by selecting each node
        for single_node in initial_node.expand(problem):
            single_state = single_node.state

            # If condition is true then return the single_node
            if problem.goal_test(single_state):
                return single_node, explored
            # If not in explored append at the front
            if tuple(single_state) not in explored:
                frontier.append(single_node)
    return None, None
...
def depth_first_graph_search(problem):
    ...
    while not is_empty(frontier):
        ...
        # going through all nodes by selecting each node
        for single_node in initial_node.expand(problem):
            # checking if it is not in explored and frontier
            if tuple(single_node.state) not in explored and single_node not in frontier:
                if problem.goal_test(single_node.state):
                    # returning the node which meets the condition and explored
                    return single_node, explored
                frontier.append(single_node)
...
def best_first_graph_search(problem, f=None):
    ...
    explored = set()
    cost_of_state = {tuple(node.state):f(node)}

    while not is_empty(frontier):
        node = frontier.pop()

        # If condition is true then return the node and explored
        ...
        # going through all nodes by selecting each node
        for single_node in node.expand(problem):
            single_state = tuple(single_node.state)

            if single_state not in explored:
                new_cost_of_state = f(single_node)

                if single_state not in cost_of_state or new_cost_of_state < cost_of_state[single_state]:
                    cost_of_state[single_state] = new_cost_of_state
                    frontier.append(single_node)
